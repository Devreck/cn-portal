...

{
  ...
  gabarito: 'C',
  explicacao: 'O pai é fenotipicamente normal, portanto tem genótipo XY — seu único X é normal. Toda filha herda obrigatoriamente esse X paterno. Para ser afetada por herança recessiva ligada ao X, a filha precisaria de dois alelos mutantes (XᵃXᵃ), recebendo Xᵃ do pai e da mãe. Mas o pai normal (XY) nunca pode transmitir Xᵃ à filha. Logo, herança ligada ao X é impossível aqui. A única explicação é herança autossômica recessiva: pais Aa × Aa podem gerar filha aa com probabilidade 1/4.',
  steps: [
    {
      titulo: 'Analisar a herança recessiva ligada ao X',
      hint: 'Filha afetada precisaria de Xᵃ do pai, mas o pai é normal',
      explicacao: 'Para herança recessiva ligada ao X, a filha afetada teria genótipo XᵃXᵃ. Ela recebe um X do pai e um X da mãe. Se o pai é fenotipicamente normal (XY), seu X é necessariamente o alelo normal — ele não pode transmitir Xᵃ a nenhuma filha.',
      linhas_latex: ['\\text{Pai normal: } XY \\rightarrow \\text{filha recebe X normal do pai}', '\\text{Filha } X^aX^a \\text{ é impossível se o pai é normal}'],
    },
    {
      titulo: 'Analisar a herança autossômica recessiva',
      hint: 'Pais Aa × Aa podem gerar aa',
      explicacao: 'Se o gene for autossômico recessivo, pais normais (Aa) portadores podem ter filha afetada (aa). Não há restrição de sexo — o alelo está em autossomo, não no X.',
      linhas_latex: ['Aa \\times Aa \\rightarrow AA : Aa : Aa : aa = 1:2:1', 'P(aa, \\text{filha afetada}) = \\frac{1}{4}'],
    },
    {
      titulo: 'Conclusão',
      hint: 'Pai normal descarta X-linked; filha afetada confirma autossômico recessivo',
      explicacao: 'Com pai normal (XY), herança ligada ao X é impossível para uma filha afetada. O gene é obrigatoriamente autossômico recessivo — a filha recebeu o alelo recessivo de ambos os pais portadores.',
      linhas_latex: ['\\text{Pai normal } (XY) \\Rightarrow \\text{herança ligada ao X descartada}', '\\text{Gene obrigatoriamente autossômico recessivo}'],
      destaque_latex: '\\boxed{\\text{Alternativa C — autossômico recessivo}}',
    },
  ],
},

...

{
  ...
  enunciado: 'Considerando a via metabólica do milho do Texto IV da AV4, onde a epistasia recessiva dupla determina que apenas grãos A_B_ produzem pigmento púrpuro, qual é a probabilidade de um grão incolor (branco) no cruzamento AaBb × AaBb?',
  alternativas: {
    A: '1/16',
    B: '3/16',
    C: '7/16',
...

{
  ...
  tema: 'Transgenia',
  enunciado: 'O desenvolvimento de variedades de cana-de-açúcar resistentes ao estresse hídrico pode ser alcançado por meio da inserção de genes de outras espécies no genoma da planta. Essa técnica é denominada:',
  alternativas: {
    A: 'Mutação induzida',
    B: 'Clonagem reprodutiva',
    C: 'Transgenia',
    D: 'Poliploidia artificial',
    E: 'Hibridização somática'
  },
  gabarito: 'C',
  explicacao: 'Transgenia é a técnica de inserção de genes exógenos (de outra espécie) no genoma de um organismo, gerando o chamado organismo geneticamente modificado (OGM) ou transgênico.',
  steps: [
    {
      titulo: 'Identificar a técnica descrita',
      hint: 'Inserção de genes de outra espécie',
      explicacao: 'A inserção de um gene exógeno (de origem diferente) no genoma de um organismo é a definição de transgenia.',
      linhas_latex: ['\\text{Gene exógeno} \\xrightarrow{\\text{vetor}} \\text{genoma hospedeiro}', '\\rightarrow \\text{Organismo Transgênico}'],
    },
  ],
},
...

{
  ...
  tema: 'Heredograma — identificação de herança',
  enunciado: 'Em um heredograma, dois pais fenotipicamente normais têm uma filha afetada por uma síndrome recessiva. O pai afetado não existe na geração parental. Com base apenas nessa informação, qual conclusão é correta sobre a localização cromossômica desse gene?',
  alternativas: {
    A: 'O gene é obrigatoriamente ligado ao cromossomo Y',
    B: 'O gene pode ser autossômico recessivo ou recessivo ligado ao X, dependendo do sexo da filha afetada',
    C: 'O gene é obrigatoriamente autossômico recessivo, pois filha afetada recebeu alelo de ambos os pais normais',
    D: 'O gene é ligado ao X, pois apenas o pai transmite X às filhas',
    E: 'Não é possível determinar o padrão de herança sem informações sobre outras gerações'
  },
  gabarito: 'B',
  explicacao: 'Uma filha afetada com pais normais pode ocorrer em herança autossômica recessiva (pais Aa × Aa) OU em herança recessiva ligada ao X (mãe portadora XᴬX × pai normal XY, filha XᴬXᴬ). Para distinguir, são necessárias mais informações sobre a família.',
  steps: [
    {
      titulo: 'Analisar herança autossômica recessiva',
      hint: 'Pais Aa × Aa → filha aa',
      explicacao: 'Se o gene for autossômico recessivo, pais normais (Aa) podem ter filha afetada (aa) com probabilidade 1/4.',
      linhas_latex: ['Aa \\times Aa \\rightarrow \\frac{1}{4} \\; aa \\; (\\text{afetada})'],
    },
    {
      titulo: 'Analisar herança recessiva ligada ao X',
      hint: 'Mãe portadora XᴬX, pai normal XY → filha XᴬXᴬ',
      explicacao: 'Se o gene for ligado ao X, a filha afetada precisa herdar o alelo mutante do pai (Xᴬ) e da mãe portadora (Xᴬ). O pai teria que ser afetado para dar Xᴬ à filha. Se o pai é normal, esse cenário é impossível.',
      linhas_latex: ['\\text{Pai normal: } XY \\rightarrow \\text{não passa } X^a \\text{ à filha}'],
    },
    {
      titulo: 'Conclusão',
      hint: 'Pai normal não pode dar Xᴬ à filha',
      explicacao: 'Como o pai normal (XY) não possui o alelo recessivo ligado ao X, a filha afetada só é possível se a herança for autossômica. Logo, o gene é autossômico recessivo.',
      linhas_latex: ['\\text{Gene autossômico recessivo}'],
      destaque_latex: '\\boxed{\\text{Alternativa B}} \\text{ — necessário mais dados para confirmar}',
    },
  ],
},
...

{
  ...
  gabarito: 'C',
  explicacao: 'A mulher é provavelmente portadora (XʰX), pois seu irmão é hemofílico (XʰY). O cruzamento XʰX × XY gera: XʰX, XX, XʰY, XY. A probabilidade de filho hemofílico (XʰY) é 1/4 = 25%.',
  steps: [
    {
      titulo: 'Determinar genótipo da mãe',
      hint: 'Irmão hemofílico → mãe é portadora',
      explicacao: 'Se o irmão é hemofílico (XʰY), ele recebeu Xʰ da mãe. A mãe da questão é filha desta mulher portadora, portanto tem 50% de chance de ser portadora. Assumindo que ela é portadora (mais provável clinicamente):',
      linhas_latex: ['\\text{Mãe: } X^hX \\text{ (portadora)}', '\\text{Pai: } XY \\text{ (normal)}'],
    },
    {
      titulo: 'Calcular a descendência',
      hint: 'Cruzamento XʰX × XY',
      explicacao: 'Os filhos possíveis são: XʰX (portadora), XX (normal), XʰY (hemofílico), XY (normal).',
      linhas_latex: ['X^hX \\times XY \\rightarrow X^hX : XX : X^hY : XY'],
    },
    {
      titulo: 'Calcular P(hemofílico)',
      hint: 'XʰY = 1/4',
      explicacao: 'Apenas XʰY é hemofílico. É 1 dos 4 resultados.',
      linhas_latex: ['P(\\text{hemofílico}) = \\frac{1}{4} = 25\\%'],
      destaque_latex: '\\boxed{25\\%} \\quad \\text{Alternativa C}',
    },
  ],
},
